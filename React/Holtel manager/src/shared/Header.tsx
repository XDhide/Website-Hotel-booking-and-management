...
import { apiLogin, apiRegister } from "../services/AccountService";
import {
  saveAuth,
  clearAuth,
  getUser,
...
export default function Header() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const roles = decodeJwtRoles(res.token);

      saveAuth(
        res.token,
        { userName: res.userName ?? username, email: res.email ?? "" },
        roles,
      );
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      saveAuth(
        res.token,
        { userName: res.userName ?? username, email: res.email ?? email },
        roles,
      );
      ...
      if (Array.isArray(errs)) {
        setError(errs.map((e: any) => e.description).join(", "));
      } else {
        setError(typeof errs === "string" ? errs : "Đăng ký thất bại");
      }
    } finally {
      setLoading(false);
    }
  ...
  const goAdmin = () => {
    ...
    else alert("Bạn không có quyền Admi");
    setDropdownOpen(false);
  };
  const goManager = () => {
    if (isManager()) navigate("/manager");
    else alert("Bạn không có quyền Manager");
    setDropdownOpen(false);
  };
    ...
    <>
      <header className="header">
        <div className="header-inner">
          ...
          >
            <span className="logo-text">DHOME</span>
          </a>
              ...
              <div className="avatar-wrapper" ref={dropdownRef}>
                ...
                {dropdownOpen && (
                  <div className="dropdown-menu">
                    ...
                    {isManager() && (
                      <button className="dropdown-item" onClick={goManager}>
                        🛡 Trang Admin
                      </button>
                    )}
                    {}
