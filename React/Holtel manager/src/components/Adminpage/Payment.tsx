import {
  apiSearch as apiSearchInvoice,
  apiGetInvoiceWithDetails,
  apiAddService,
  apiGetServices,
  apiPayInvoice,
} from "../../services/InvoiceService";
import { apiSearchDiscount } from "../../services/DiscountService";
import {
  SearchOutlined,
  ReloadOutlined,
  PlusOutlined,
...
export default function Payment() {
  ...
  const [showConfirmSvc, setShowConfirmSvc] = useState(false);
  ...
  const load = useCallback(async (p: number) => {
    setLoading(true);
    try {
      const res = await apiSearchInvoice(p, PAGE_SIZE);
      const raw = res.data;
  ...
  const loadServices = async () => {
    try {
      ...
      setServices(Array.isArray(res) ? res : ((res as any)?.data ?? []));
    } catch {
      setServices([]);
    }
  };
  ...
  const openDetail = async (inv: Invoice) => {
    ...
    setDetail(inv);
    try {
      const res = await apiGetInvoiceWithDetails(inv.invoiceId);
      setDetail(res);
    } catch {
    } finally {
  ...
  const checkVoucher = async () => {
    ...
    setVoucherInfo(null);
    try {
      const res = await apiSearchDiscount(1, 100);
      const list = res.data?.data ?? res.data ?? [];
      const found = list.find(
        (d: any) =>
          (d.name ?? "").toLowerCase() === voucherCode.trim().toLowerCase() &&
          d.isActive !== false,
      );
      if (!found) {
        setVoucherErr("Mã giảm giá không hợp lệ hoặc đã hết hạn");
        return;
      }
      ...
      if (
        ...
        if (found.maxDiscount)
          discountAmt = Math.min(discountAmt, found.maxDiscount);
      } else {
        discountAmt = found.discountValue ?? 0;
      }
      if (found.minAmount && detailSubTotal < found.minAmount) {
        setVoucherErr(
          `Hóa đơn tối thiểu ${fmt(found.minAmount)} để dùng mã này`,
        );
        return;
      }
      ...
      setVoucherErr("Không thể kiểm tra mã giảm giá");
    } finally {
  ...
  const handlePay = async () => {
    ...
    setPaying(true);
    try {
      const res = await apiPayInvoice(detail.invoiceId, {
        discountAmount: payDiscount,
        surchargeAmount: paySurcharge,
        paymentMethod: payMethod,
        note: payNote || "",
      });
      setDetail({ ...detail, ...res, paymentStatus: "Paid" });
  ...
  const detailSubTotal =
    detail?.invoiceDetails?.reduce((s, d) => s + (d.totalPrice ?? 0), 0) ??
    detail?.subTotal ??
    0;
  const detailDeposit = detail?.deposit ?? 0;
  const previewFinal = Math.max(
    0,
    detailSubTotal - detailDeposit - payDiscount + paySurcharge,
  );
    ...
    <div className="payment-wrapper">
      ...
      <div className="pmt-toolbar">
        <div className="pmt-tabs">
          {(["all", "unpaid", "paid"] as const).map((t) => (
            <button
              key={t}
              className={`pmt-tab${tab === t ? " active" : ""}`}
              onClick={() => setTab(t)}
            >
        ...
        <div className="pmt-search">
          <SearchOutlined />
          <input
            placeholder="Tìm mã HĐ, phòng..."
            value={search}
            onChange={(e) => setSearch(e.target.value)}
          />
        </div>
        <button className="pmt-reload-btn" onClick={() => load(page)}>
          <ReloadOutlined />
        </button>
      </div>
            ...
            {loading ? (
              ...
              filtered.map((inv) => {
                ...
                return (
                  ...
                  >
                    <td>
                      <span className="pmt-id">#{inv.invoiceId}</span>
                    </td>
                    <td>
                      <span className="pmt-room">
                        {inv.roomNumber ? (
                          <>
                            <HomeOutlined className="pmt-room-icon" />
                    ...
                    <td>
                      <span className="pmt-room-type-text">
                        {inv.roomTypeName ?? "—"}
                      </span>
                    </td>
                    ...
                    <td className="pmt-amount">
                      {isPaid ? (
                        ...
                        <span className="pmt-amount-pending">
                          {fmt(inv.currentTotal ?? inv.subTotal ?? 0)}
                        </span>
                      )}
                    </td>
                    <td>
                      <span
                        className={`pmt-badge ${isPaid ? "paid" : "unpaid"}`}
                      >
                        {isPaid ? (
                          <>
      ...
      {totalPages > 1 && (
        <div className="pmt-pagination">
          <span className="pmt-page-info">
            Trang {page}/{totalPages} — {totalCount} hóa đơn
          </span>
        ...
        <div className="pmt-modal-overlay" onClick={() => setDetail(null)}>
          <div className="pmt-modal" onClick={(e) => e.stopPropagation()}>
            <div className="pmt-modal-header">
              <div>
                <div className="pmt-modal-title">
                  Hóa đơn #{detail.invoiceId}
                </div>
                <div className="pmt-modal-sub">
                  {detail.roomNumber && (
                    <span>
                      <HomeOutlined /> Phòng {detail.roomNumber}
                    </span>
                  )}
            ...
            {loadingDetail ? (
              <div className="pmt-detail-loading">
                <LoadingOutlined /> Đang tải chi tiết...
              </div>
            ) : (
              <div className="pmt-modal-body">
                      ...
                      <tbody>
                        {detail.invoiceDetails.map((d) => (
                          <tr key={d.invoiceDetailId}>
                            ...
                            <td>
                              <span
                                className={`pmt-item-type ${d.itemType?.toLowerCase()}`}
                              >
                                {d.itemType === "Room" ? (
                                  <>
                                    <HomeOutlined /> Phòng
                ...
                <div className="pmt-summary-box">
                  {[
                    ...
                    [
                      "Tiền cọc (đã trả)",
                      detailDeposit > 0 ? `-${fmt(detailDeposit)}` : "—",
                    ],
                    [
                      "Giảm giá",
      ...
      {showAddSvc && detail && (
        <div className="pmt-modal-overlay" onClick={() => setShowAddSvc(false)}>
          <div
            className="pmt-modal pmt-modal-sm"
                ...
                {services.length === 0 ? (
                  ...
                  services.map((svc) => (
                    ...
                    >
                      ...
                      <div className="pmt-svc-meta">
                        {svc.serviceType} · {fmt(svc.price)}/{svc.unit}
                      </div>
                    </div>
                  ))
                )}
              ...
              {selSvc && (
                <div className="pmt-svc-qty-row">
                  ...
                  <input
                    ...
                    value={svcQty}
                    onChange={(e) =>
                      setSvcQty(Math.max(1, Number(e.target.value)))
                    }
                  />
                  <span className="pmt-svc-total">
                    {fmt(selSvc.price * svcQty)}
                  </span>
                </div>
              )}
              <button
                className="pmt-btn-pay pmt-w-100 pmt-mt-12"
                disabled={!selSvc}
                onClick={handleAddService}
              >
                <>
                  <CustomerServiceOutlined /> Xem & Xác nhận dịch vụ
                </>
              </button>
            </div>
          </div>
      ...
      {showConfirmSvc && selSvc && detail && (
        <div
          className="pmt-modal-overlay"
          onClick={() => setShowConfirmSvc(false)}
        >
          <div
            className="pmt-modal pmt-modal-sm"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="pmt-modal-header">
              <div className="pmt-modal-title">
                <CustomerServiceOutlined /> Xác nhận gọi dịch vụ
              </div>
              <button
                className="pmt-btn-close"
                onClick={() => setShowConfirmSvc(false)}
            ...
            <div className="pmt-modal-body">
              <div className="pmt-confirm-info-box">
                ...
                {[
                  ...
                  ["Đơn giá", fmt(selSvc.price)],
                  ["Số lượng", String(svcQty)],
                  ["Thành tiền", fmt(selSvc.price * svcQty)],
                  ["Hóa đơn", `#${detail.invoiceId}`],
                ].map(([l, v]) => (
                  <div key={l} className="pmt-confirm-row">
                    <span className="pmt-confirm-label">{l}</span>
                    <span className="pmt-confirm-value">{v}</span>
                  </div>
                ))}
              </div>
              ...
              <div className="pmt-confirm-actions">
                <button
                  className="pmt-btn-cancel-modal"
                  onClick={() => setShowConfirmSvc(false)}
                >
                  Hủy
                </button>
                <button
                  className="pmt-btn-pay pmt-btn-confirm-modal"
                  disabled={addingSvc}
                ...
                >
                  {addingSvc ? (
                    ...
                    <>
                      <CheckCircleOutlined /> Xác nhận & Thêm vào HĐ
                    </>
                  )}
                </button>
              </div>
      ...
      {showPayModal && detail && (
        ...
        >
          ...
          >
            <div className="pmt-modal-header">
              <div className="pmt-modal-title">
                <DollarOutlined /> Xác nhận thanh toán
              </div>
              <button
                className="pmt-btn-close"
                onClick={() => setShowPayModal(false)}
            ...
            <div className="pmt-modal-body">
              <div className="pmt-pay-form">
                ...
                <div className="pmt-voucher-wrap">
                  <input
                    type="text"
                    value={voucherCode}
                    placeholder="Nhập mã voucher..."
                    className="pmt-voucher-input"
                    onChange={(e) => {
                  ...
                  <button
                    ...
                    className="pmt-voucher-btn"
                  >
                    {checkingVoucher ? "..." : "Áp dụng"}
                  </button>
                </div>
                {voucherErr && (
                  <div className="pmt-voucher-err">{voucherErr}</div>
                )}
                {voucherInfo && (
                ...
                <input
                  ...
                  value={payDiscount}
                  onChange={(e) => {
                    setPayDiscount(Number(e.target.value));
                    setVoucherInfo(null);
                    setVoucherCode("");
                  }}
                />
              ...
              <div className="pmt-pay-preview">
                <div>
                  <span>Tạm tính</span>
                  <span>{fmt(detailSubTotal)}</span>
                </div>
                {detailDeposit > 0 && (
                  <div>
                    <span>Trừ cọc</span>
                    <span>-{fmt(detailDeposit)}</span>
                  </div>
                )}
                {payDiscount > 0 && (
                  <div>
                    <span>Giảm giá</span>
                    <span>-{fmt(payDiscount)}</span>
                  </div>
                )}
                {paySurcharge > 0 && (
                  <div>
                    <span>Phụ thu</span>
                    <span>+{fmt(paySurcharge)}</span>
                  </div>
                )}
                <div className="pmt-pay-final">
                  <span>Tổng thanh toán</span>
                  <span>{fmt(previewFinal)}</span>
                </div>
              </div>
              <button
                className="pmt-btn-pay pmt-w-100"
                disabled={paying}
                onClick={handlePay}
              >
                {paying ? (
