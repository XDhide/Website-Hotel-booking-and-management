...
export default function RoomRateManager() {
  ...
  const loadRates = useCallback(async () => {
    setLoading(true);
    try {
      const res = await apiGetAllRoomRates(1, 200);
      const raw = res.data;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editId) {
        await apiUpdateRoomRate(editId, payload);
      } else {
        await apiCreateRoomRate(payload.roomTypeId, payload);
      }
      setShowForm(false);
      await loadRates();
    } catch (e: any) {
    ...
    <div className="rrm-wrapper">
      ...
      <div className="rrm-header">
        <div>
          <h2 className="rrm-title">
            <TagsOutlined /> Quản lý giá phòng
          </h2>
          <p className="rrm-sub">
            Tạo nhiều mức giá theo loại phòng và thời điểm
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="rrm-empty">
          <TagsOutlined className="rrm-empty-icon" />
          <p>Chưa có mức giá nào</p>
          <button className="rrm-btn-add" onClick={openCreate}>
            <PlusOutlined /> Thêm mức giá đầu tiên
          </button>
        </div>
      ) : (
        <div className="rrm-grid">
          {filtered.map((r) => {
            ...
            return (
              <div
                key={r.roomRateId}
                className={`rrm-card${r.isActive ? "" : " inactive"}`}
              >
                <div className="rrm-card-top">
                  <span
                    className="rrm-type-badge"
                    style={{ background: `${color}20`, color }}
                  >
                    {RENT_LABELS[r.rentType] ?? r.rentType}
                  </span>
                  <span
                    className={`rrm-active-badge ${r.isActive ? "on" : "off"}`}
                  >
                    {r.isActive ? (
                      <>
                        <CheckCircleOutlined /> Đang dùng
                      </>
                    ) : (
                      <>
                        <StopOutlined /> Tắt
                ...
                <div className="rrm-card-price" style={{ color }}>
                  {fmt(r.price)}
                  <span className="rrm-card-unit">
                    / {RENT_LABELS[r.rentType]?.toLowerCase() ?? r.rentType}
                  </span>
                </div>
                ...
                <div className="rrm-card-actions">
                  <button
                    className="rrm-action-btn edit"
                    onClick={() => openEdit(r)}
                  >
                    <EditOutlined />
                  </button>
                  <button
                    className="rrm-action-btn del"
                    disabled={deleting === r.roomRateId}
                    onClick={() => handleDelete(r.roomRateId)}
                  >
                    {deleting === r.roomRateId ? (
                      <LoadingOutlined />
                    ) : (
                      <DeleteOutlined />
                    )}
        ...
        <div className="rrm-overlay" onClick={() => setShowForm(false)}>
          <div className="rrm-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="rrm-modal-body">
              ...
              <div className="rrm-field">
                <label>
                  Loại giá <span className="req">*</span>
                </label>
            ...
            <div className="rrm-modal-footer">
              ...
              >
                {saving ? (
                  ...
                  <>
                    <CheckCircleOutlined /> Lưu mức giá
                  </>
                )}
              </button>
            </div>
