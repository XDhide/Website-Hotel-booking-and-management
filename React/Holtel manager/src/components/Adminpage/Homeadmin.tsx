...
export default function HomeAdmin() {
  ...
  const loadCatalog = useCallback(async () => {
    setLoadingCat(true);
    try {
      const [rtRes] = await Promise.all([
        apiClient.get(`${API}/roomtype?page=1&limit=100`),
      ]);
      const norm = (r: any) =>
        Array.isArray(r.data) ? r.data : (r.data?.data ?? []);
      setRoomTypes(
        norm(rtRes).map((rt: any) => ({
          id: rt.id ?? rt.Id,
  ...
  const handleCreateBooking = async () => {
    if (!selectedRT) {
      showToast("Vui lòng chọn loại phòng", false);
      return;
    }
    if (!checkIn) {
      showToast("Vui lòng chọn ngày nhận phòng", false);
      return;
    }
    if (!checkOut) {
      showToast("Vui lòng chọn ngày trả phòng", false);
      return;
    }
    if (new Date(checkOut) <= new Date(checkIn)) {
      showToast("Ngày trả phòng phải sau ngày nhận phòng", false);
      return;
    }
    setSaving(true);
    try {
      ...
      showToast(
        `✓ Booking #${bId} đã tạo! Chọn booking và nhấn Check-in để xác nhận khách vào phòng.`,
      );
      resetWizard();
  ...
  const handleCheckIn = async () => {
    ...
    try {
      await apiClient.post(`${API}/CheckInOut/checkin`, {
        bookingId: checkInBk.id,
      });
      showToast(
        `Check-in booking #${checkInBk.id} thành công! Hóa đơn đã được tạo.`,
      );
      setCheckInBk(null);
  ...
  const availableRTs = roomTypes.filter(
    (rt) =>
      rt.availableRooms > 0 &&
      rt.name.toLowerCase().includes(rtSearch.toLowerCase()),
  );
  ...
  const filteredInv = invoices.filter(
    (inv) =>
      String(inv.roomUseId).includes(invSearch) ||
      String(inv.invoiceId).includes(invSearch),
  );
  ...
  return (
    <div className="ha-container">
      {toast && (
        <div className={`ha-toast ${toast.ok ? "ok" : "err"}`}>{toast.msg}</div>
      )}
      ...
      <div className="stats-bar">
        ...
        <button
          className="stats-create-btn"
          onClick={() => setStep("pickRoom")}
        >
        ...
        <div className="booking-table-header">
          ...
          <div className="booking-table-controls">
            {(["pending", "active", "all"] as const).map((t) => (
              <button
                key={t}
                className={`booking-tab-btn ${bkTab === t ? "active" : "inactive"}`}
                onClick={() => setBkTab(t)}
              >
                {t === "pending"
                  ? "Chờ xác nhận"
            ...
            <button
              className="booking-reload-btn"
              onClick={() => loadBookings(bkPage)}
            >
              <ReloadOutlined />
            </button>
          </div>
        ...
        {bkTotalPg > 1 && (
          <div className="booking-table-pagination">
            {Array.from({ length: bkTotalPg }, (_, i) => i + 1).map((p) => (
              <button
                key={p}
                className={`booking-page-btn ${p === bkPage ? "active" : "inactive"}`}
                onClick={() => setBkPage(p)}
              >
                {p}
              </button>
      ...
      <div className="unpaid-table-wrap">
        <div className="unpaid-table-header">
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button className="unpaid-reload-btn" onClick={loadInvoices}>
              <ReloadOutlined />
            </button>
          </div>
        </div>
        {loadingInv ? (
          ...
          <div className="unpaid-table-empty">
            <CheckCircleOutlined className="unpaid-table-empty-icon" /> Không có
      ...
      {step === "pickRoom" && (
        <Modal
          ...
          onClose={resetWizard}
          onSave={() => selectedRT && setStep("booking")}
        >
          <div className="bw-modal-col">
            ...
            {loadingCat ? (
              ...
              <div className="bw-empty">
                <HomeOutlined className="bw-empty-icon" />
                ...
                {availableRTs.map((rt) => {
                  ...
                  return (
                    <div
                      ...
                      className={`bw-card ${isSelected ? "active" : ""}`}
                    >
                      {cover ? (
                        <img
                          src={cover}
            ...
            {selectedRT && (
              <div className="bw-selected-info">
                <CheckOutlined className="bw-icon-margin" />
                Đã chọn: {selectedRT.name} ({selectedRT.availableRooms} phòng
                trống)
              </div>
            )}
          </div>
      ...
      {step === "booking" && selectedRT && (
        <Modal
          title={`Booking — ${selectedRT.name}`}
          onClose={resetWizard}
          onSave={handleCreateBooking}
        >
          <div className="bw-modal-col">
            <div className="bw-selected-info">
              <HomeOutlined className="bw-icon-margin" />
              {selectedRT.name} — {selectedRT.availableRooms} phòng trống
            </div>
            <div className="bw-info-alert">
              ℹ️ Sau khi tạo booking, vào danh sách booking và nhấn{" "}
              <strong>Check-in</strong> để xác nhận khách vào phòng và tạo hóa
              đơn.
            </div>
      ...
      {checkInBk && (
        <Modal
          title={`Check-in Booking #${checkInBk.id}`}
          onClose={() => setCheckInBk(null)}
        ...
        >
          <div className="bw-modal-col">
            ...
            {[
              ...
              [
                "Loại phòng",
                checkInBk.roomTypeName ?? `Loại #${checkInBk.roomTypeId}`,
              ],
