import {
  ...
  apiUpdateRoomType,
  apiDeleteRoomType,
  apiUploadRoomTypeFile,
  apiDeleteRoomTypeImage,
...
export default function RoomTypeManager() {
  ...
  const uploadFile = async (roomTypeId: number, file: File, order: number) => {
    ...
    await apiUploadRoomTypeFile(roomTypeId, fd);
  };

  const handleSave = async () => {
    if (!form.name.trim()) {
      setFormErr("Vui lòng nhập tên loại phòng.");
      return;
    }
    setSaving(true);
    setFormErr("");
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const imgData = await apiUploadRoomTypeFile(imgTarget.id, fd);
      const updated = { ...imgTarget, images: [...imgTarget.images, imgData] };
  ...
  const handleDeleteImg = async (imgId: number) => {
    if (!imgTarget) return;
    try {
      await apiDeleteRoomTypeImage(imgTarget.id, imgId);
      const updated = {
        ...imgTarget,
        images: imgTarget.images.filter((i) => i.id !== imgId),
      };
      setImgTarget(updated);
      setItems((prev) => prev.map((r) => (r.id === updated.id ? updated : r)));
    } catch {
    ...
    <div className="rtm-wrap">
        ...
        {loading ? (
          ...
          <table className="rtm-table">
            ...
            <tbody>
              {filtered.map((rt, i) => {
                const cover =
                  rt.images?.find((img) => img.displayOrder === 0) ??
                  rt.images?.[0];
                return (
                  <tr key={rt.id}>
                    ...
                    <td>
                      <span
                        className={`rtm-avail${(rt.availableRooms ?? 0) > 0 ? " ok" : " none"}`}
                      >
                    ...
                    <td>
                      <button
                        className="rtm-img-btn"
                        onClick={() => openImages(rt)}
                      >
                    ...
                    <td>
                      <div className="rtm-actions">
                        <button
                          className="rtm-act-btn edit"
                          onClick={() => openEdit(rt)}
                        >
                          <EditOutlined />
                        </button>
                        <button
                          className="rtm-act-btn del"
      ...
      {totalPages > 1 && (
        <div className="rtm-pagination">
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
            <button
              key={p}
              className={`rtm-page-btn${p === page ? " active" : ""}`}
              onClick={() => setPage(p)}
            >
              {p}
            </button>
            ...
            <div className="rtm-modal-footer">
              ...
              >
                {saving ? (
                  uploadingIdx !== null ? (
                    <>
                      <LoadingOutlined /> Đang upload ảnh {uploadingIdx + 1}/
                      {pendingFiles.length}...
                    </>
                  ) : (
                    <>
      ...
      {imgTarget && (
        <div className="rtm-overlay" onClick={() => setImgTarget(null)}>
          <div
            className="rtm-modal rtm-img-modal"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="rtm-modal-header">
              <h3>
                <PictureOutlined /> Ảnh — {imgTarget.name}
              </h3>
              <button
                className="rtm-modal-close"
            ...
            <div className="rtm-modal-body">
              ...
              >
                {uploading ? (
                  ...
                  <>
                    <UploadOutlined /> Chọn ảnh để upload (jpg, png, webp — tối
                    đa 10MB)
                  </>
                )}
              </div>
                  ...
                  {imgTarget.images.map((img, idx) => (
                    <div key={img.id} className="rtm-img-item">
                      <img
                        src={img.imageUrl}
                        alt={img.altText}
                        className="rtm-img-preview"
                      />
                      <div className="rtm-img-overlay">
                        {idx === 0 ? (
                          <span className="rtm-img-cover-badge">
                            <StarFilled /> Ảnh bìa
                          </span>
                        ) : (
                          <span className="rtm-img-order-badge">
                            <StarOutlined /> #{idx + 1}
                          </span>
                        )}
                        <button
                          className="rtm-img-del"
                          onClick={() => handleDeleteImg(img.id)}
                        >
                          <DeleteOutlined />
                        </button>
                      </div>
                      <div className="rtm-img-alt">
              ...
              <p className="rtm-img-note">
                Ảnh đầu tiên (displayOrder=0) là ảnh bìa hiển thị ngoài trang
                chủ.
              </p>
            </div>
          </div>
