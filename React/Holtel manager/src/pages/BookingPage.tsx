...
export default function BookingPage({
  ...
  const nights =
    ciDate && coDate
      ? Math.max(
          0,
          Math.round(
            (new Date(coDate).getTime() - new Date(ciDate).getTime()) /
              86400000,
          ),
        )
      : 0;
  ...
  const pricePerNight =
    roomType?.roomRates?.[0]?.price ?? roomType?.basePrice ?? 0;
    ...
    apiSearchRoomType(1, 100)
      .then((res) => {
        const found = (res?.data ?? []).find(
          (r: any) => (r.roomTypeId ?? r.id) === roomTypeId,
        );
  ...
  const handleSubmit = async () => {
    ...
    if (roomType && (roomType.availableRooms ?? 0) <= 0) {
      setError(
        "Loại phòng này hiện không còn phòng trống. Vui lòng chọn loại phòng khác.",
      );
      return;
    }
    ...
    return (
      <>
        ...
        <div className="bk-loading">
          <LoadingOutlined className="bk-loading-icon-large" />
        </div>
        <Footer />
      </>
    );
        ...
        <div className="bk-success-wrap">
          <div className="bk-success-card">
            ...
            <p className="bk-success-sub">
              Yêu cầu đặt phòng của bạn đã được ghi nhận với trạng thái{" "}
              <strong>Chờ xác nhận</strong>. Nhân viên sẽ liên hệ xác nhận và
              thông báo thông tin phòng trong vòng 24 giờ.
            </p>
            <div className="bk-success-info">
              <div>
                <span>Loại phòng</span>
                <strong>{roomType?.name ?? `#${roomTypeId}`}</strong>
              </div>
              <div>
                <span>Nhận phòng</span>
                <strong>{ciDate}</strong>
              </div>
              <div>
                <span>Trả phòng</span>
                <strong>{coDate}</strong>
              </div>
              <div>
                <span>Số đêm</span>
              ...
              <div>
                ...
                <strong className="bk-text-yellow">Chờ xác nhận</strong>
              </div>
            </div>
            <div className="bk-success-note">
              Việc thanh toán sẽ được thực hiện tại khách sạn hoặc theo hướng
              dẫn của nhân viên.
            </div>
          ...
          <div className="bk-layout">
            ...
            <div className="bk-left">
              ...
              <div className="bk-payment-note">
                <span>💡</span>
                <span>
                  <strong>Lưu ý về thanh toán:</strong> Bạn chưa cần thanh toán
                  ngay. Nhân viên khách sạn sẽ xử lý thanh toán khi bạn nhận
                  phòng hoặc liên hệ trực tiếp với bạn.
                </span>
              </div>
            </div>
              ...
              <div className="bk-summary">
                ...
                {roomType ? (
                  <>
                    ...
                    {roomType.capacity && (
                      <div className="bk-summary-capacity">
                        {roomType.capacity}
                      </div>
                    )}
                  </>
                ) : (
                  <div className="bk-summary-name">
                ...
                <div className="bk-summary-rows">
                  ...
                  <div>
                    <span>Số đêm</span>
                    <strong>{nights > 0 ? `${nights} đêm` : "–"}</strong>
                  </div>
                  <div>
                    <span>Số khách</span>
                    <strong>{guestNum} khách</strong>
                  ...
                  {pricePerNight > 0 && (
                    <div>
                      ...
                      <strong>{pricePerNight.toLocaleString("vi-VN")}₫</strong>
                    </div>
                  )}
                </div>
                ...
                <div className="bk-status-badge">
                  Trạng thái sau đặt: <strong>Chờ xác nhận</strong>
                </div>

                {}
                {roomType && (
                  <div
                    className={`bk-avail-badge ${(roomType.availableRooms ?? 0) > 0 ? "ok" : "none"}`}
                  >
                    {(roomType.availableRooms ?? 0) > 0 ? (
                      <>
                        ✓ Còn <strong>{roomType.availableRooms}</strong> phòng
                        trống
                      </>
                    ) : (
                ...
                <button
                  ...
                  disabled={
                    submitting ||
                    (roomType && (roomType.availableRooms ?? 0) <= 0)
                  }
                >
                  {submitting ? (
                    <>
                      <LoadingOutlined className="bk-mr-8" />
                      Đang gửi...
                    </>
                  ) : (
                    <>
                      <CheckCircleOutlined className="bk-mr-8" />
                      Xác nhận đặt phòng
                    </>
                  )}
                </button>
                ...
                <p className="bk-note">
                  Chưa thanh toán — nhân viên sẽ liên hệ xác nhận.
                </p>
              </div>
            </div>
          </div>
