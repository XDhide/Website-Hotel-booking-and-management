...
import {
  ...
  BarsOutlined, SortAscendingOutlined, LoadingOutlined,
...
async function fetchRatesForRoomTypes(ids: number[]): Promise<Record<number,{day:number,hour:number}>> {
  const map: Record<number,{day:number,hour:number}> = {};
  await Promise.allSettled(ids.map(async id => {
    try {
      const res = await apiClient.get(`${API}/roomrate/by-roomtype/${id}`);
      const rates: any[] = Array.isArray(res.data) ? res.data : [];
      const active = rates.filter(r=>r.isActive!==false);
      const dayRate  = active.find(r=>r.rentType==="Day") ?? active.find(r=>r.rentType==="Night");
      const hourRate = active.find(r=>r.rentType==="Hour");
      map[id] = { day: dayRate?.price??0, hour: hourRate?.price??0 };
    } catch { map[id] = {day:0,hour:0}; }
  }));
  return map;
}

function mapRoomType(rt: any, rateMap: Record<number,{day:number,hour:number}>) {
      ...
      <div className="rl-page">
        <div className="rl-header">
          <div className="container">
            ...
            <p className="rl-sub">Khám phá {allRooms.length} loại phòng đa dạng</p>
          </div>
        </div>
        ...
        <div className="container rl-body">
          <aside className="rl-sidebar">
            <div className="rl-filter-box">
              <div className="rl-filter-title"><FilterOutlined/> Bộ lọc</div>

              <div className="rl-filter-group">
                <label className="rl-filter-label">Tìm kiếm</label>
                <div className="rl-search-wrap">
                  <SearchOutlined className="rl-search-icon"/>
                  <input className="rl-search-input" placeholder="Tên phòng..." value={search} onChange={e=>setSearch(e.target.value)}/>
                </div>
              </div>

              <div className="rl-filter-group">
                <label className="rl-filter-label">Giá tối đa: <strong>{priceMax.toLocaleString("vi-VN")}₫</strong></label>
                <input type="range" min={100_000} max={10_000_000} step={100_000}
                  value={priceMax} onChange={e=>setPriceMax(Number(e.target.value))} className="rl-range"/>
                <div className="rl-range-labels"><span>100K</span><span>10M</span></div>
              </div>

              <div className="rl-filter-group">
                <label className="rl-checkbox-label">
                  <input type="checkbox" checked={onlyAvail} onChange={e=>setOnlyAvail(e.target.checked)} className="rl-checkbox"/>
                  Chỉ phòng còn trống
                </label>
              </div>
            </div>
            ...
            <div className="rl-toolbar">
              ...
              <div className="rl-toolbar-right">
                <div className="rl-sort-wrap">
                  ...
                  <select className="rl-sort-select" value={sort} onChange={e=>setSort(e.target.value)}>
                    {SORT_OPTIONS.map(o=><option key={o.value} value={o.value}>{o.label}</option>)}
                  </select>
                </div>
                <div className="rl-view-toggle">
                  <button className={`rl-view-btn${view==="grid"?" active":""}`} onClick={()=>setView("grid")}><AppstoreOutlined/></button>
                  <button className={`rl-view-btn${view==="list"?" active":""}`} onClick={()=>setView("list")}><BarsOutlined/></button>
                </div>
              </div>
            </div>
            {loading ? (
              <div className="rl-empty"><LoadingOutlined style={{fontSize:32,marginBottom:12}}/><p>Đang tải danh sách phòng...</p></div>
            ) : rooms.length===0 ? (
              <div className="rl-empty"><div className="rl-empty-icon"><SearchOutlined/></div><p>Không tìm thấy phòng phù hợp</p></div>
            ) : (
              <div className={view==="grid"?"room-grid":"rl-list"}>
                {rooms.map(r=><RoomCard key={r.id} room={r} compact={view==="grid"}/>)}
              </div>
            )}
          </main>
        </div>
